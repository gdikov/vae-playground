...
from playground.utils.datasets import load_conjoint_synthetic, load_mnist, load_usps
...
def run_usps_mnist_experiment(model='avb', pretrained_model=None):
    logger.info("Starting a conjoint model experiment on the USPS and MNIST dataset.")
    data_dims = (784, 256)
    latent_dims = (2, 2, 2)
    data_0 = load_mnist(one_hot=False, binarised=False, background=None, rotated=False)
    data_1 = load_usps(one_hot=False)
    ...
    if model == 'vae':
        ...
        trainer = ConjointAVBModelTrainer(data_dims=data_dims, latent_dims=latent_dims, noise_dim=64,
                                          ...
                                          overwrite=True, save_best=True,
                                          pretrained_dir=pretrained_model,
                                          architecture='usps_mnist',
                                          experiment_name='usps_mnist')
    else:
        raise ValueError("Currently only `avb` and `vae` are supported.")
    ...
    latent_vars = trained_model.infer(train_data, batch_size=100, sampling_size=sampling_size, group_by_target=True)
    save_array(path_join(model_dir, 'latent_samples.npy'), latent_vars)
    plot_latent_2d(latent_vars[:, -2:], repeat(train_data[0]['target'], sampling_size),
                   fig_dirpath=model_dir, fig_name='shared.png')
    stop_id = 0
